...
function parseSimpleString(buffer, index) {
  const start = index;
  let end = start;
  while (buffer[index++] != 10)
    end++;
  const result = buffer.toString('utf8', start, end - 1);
  return [result, index - 1]
}

function parseError(buffer, index) {
  throw parseSimpleString(buffer, index)[0];
}
...
function parseBulkString(buffer, index) {
  ...
  const result = buffer.toString('utf8', start, end);  
  return [result, end + 1];
}
...
function parseValues(buffer, index) {
  switch(buffer[index]) {
    case 43: // '+'
      return parseSimpleString(buffer, index + 1)
    case 45: // '-'
