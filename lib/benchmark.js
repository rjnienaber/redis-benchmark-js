...
class Benchmark {
  ...
  async run() {
    ...
    this.requestsInMilliseconds = Array(1000).fill(0)
  ...
  calculateResponseTime(diff) {
    const elapsedMilliseconds = Math.ceil(((diff[0] * 1e9) + diff[1]) / 1e6);
    this.requestsInMilliseconds[elapsedMilliseconds] += 1
  }
}
